...
- 0 = עובדתית, או שאין לה השפעה ברורה על השוק. מדריך, הסבר או ניתוח נתונים בלי אירוע חדש הם תמיד 0.
...
export type AskFormat = "whatsapp" | "paragraph" | "article";
...
export async function formatAskAnswer(params: {
  ...
  // Caller decides how many sources each format deserves; this is only a
  // backstop so a caller passing hundreds cannot blow up the prompt.
  const list = sources
    .slice(0, 20)
    .map((s, i) => `[${i + 1}] ${s.title}${s.source ? ` (${s.source})` : ""}`)
    .join("\n");

  const INSTRUCTIONS: Record<AskFormat, string> = {
    whatsapp: `הפוך את התשובה להודעת וואטסאפ שבן שולח לקבוצה.

- **השורה הראשונה היא הוק.** משפט אחד שגורם לעצור ולקרוא: מספר חריג, ניגוד, או משהו שנשמע לא הגיוני עד שקוראים הלאה.
- **אסור לפתוח בביטוי זמן.** לא "בחודש האחרון", לא "באוגוסט", לא "לאחרונה". ההודעה נפתחת בעניין עצמו.
- בלי "היי" ובלי הקדמות.
- 3 עד 5 שורות אחרי ההוק, כל אחת עם מספר או עובדה. לא פסקה אחת ארוכה.
...
- העברית צריכה להישמע כמו הודעה שנכתבה מהר, לא כמו מאמר.`,
    ...
    paragraph: `הפוך את התשובה לפסקה שאפשר להדביק לתוך כתבה.
...
- בלי מסקנה מסכמת ובלי המלצות. תיאור בלבד.`,
    ...
    article: `הפוך את התשובה לטיוטת כתבה קצרה.

מבנה:
- שורה ראשונה: כותרת. עובדתית וספציפית, עם המספר או השם המרכזי. בלי סימן קריאה.
- שורה שנייה: כותרת משנה במשפט אחד.
- אחר כך 3 עד 4 פסקאות. הראשונה עונה מה קרה, השאר מפרטות.

כללים:
- **רק מה שמופיע בתשובה ובמקורות.** אסור להוסיף רקע, הערכה או תחזית משלך.
- לשמור על כל המספרים, השמות והתאריכים בדיוק.
- לייחס בתוך הטקסט ("לפי גלובס", "בדיווח של כלכליסט"), בלי מספרים בסוגריים.
- בלי מקפים ארוכים, בלי נקודות תבליט, בלי סיכום שחוזר על מה שנאמר.`,
  };
